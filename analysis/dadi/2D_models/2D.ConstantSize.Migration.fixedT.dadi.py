...
modelName="2D.ConstantSize.Migration.fixedT"
...
def split_Wmig(params, ns, pts): 
    ...
    phi = Integration.two_pops(phi, xx, 3.6e-4, nu1, nu2, m12=m, m21=m)  # two pops at diff sizes with symmetric migration
    # allow for another size change; fix time at 35 gen
    fs = Spectrum.from_phi(phi, ns, (xx,xx)) 
    return fs
param_names=("nu1","nu2","m")
...
upper_bound = [1, 1, 20000] # 20 as upper bound on mig rec by dadi
p0 = [0.1, 0.1, 2000] # initial parameters; changing starting mu parameter because won't get permuted 
...
m_fraction=popt[2]/(2*Nanc) # migration rates come out scaled by fraction of members of pop  scaled * 2Na according to dadi manual so here you actually DIVIDE by 2Na *not multiply* to get the migration fraction. Tricky tricky :)
# THIS IS TRICKY: confirmed here: https://groups.google.com/forum/#!topic/dadi-user/AY_4NoEjsx8
scaled_param_names=("Nanc_FromTheta_scaled_dip","nu1_scaled_dip","nu2_scaled_dip","migrationFraction")
scaled_popt=(Nanc,nu1_scaled_dip,nu2_scaled_dip,m_fraction)
