...
modelName="2D.3epoch.migration.fixedT"
...
def split_3epoch_mig(params, ns, pts): 
    nu1,t1,nuCP,nuGCT,m = params 
    ...
    phi = Integration.two_pops(phi, xx, 3.6e-4, nuCP, nuCP, m12=m, m21=m)  # two pops at diff sizes with symmetric migration
    fs = Spectrum.from_phi(phi, ns, (xx,xx)) 
    return fs
param_names=("nu1","t1","nuCP","nuGCT","m")

lower_bound = [0.9, 1e-5, 1e-4, 1e-3, 1e-5]
upper_bound = [1.1, 1e-3, 1e-2, 0.1, 5000] 
p0 = [1, 1, 8e-4, 3e-3, 3000] # initial parameters


func=split_3epoch_mig # set the functiion
...
nuCP_scaled_dip=popt[2]*Nanc
nuGCT_scaled_dip=popt[3]*Nanc
m_fraction=popt[4]/(2*Nanc)
...
scaled_param_names=("Nanc_FromTheta_scaled_dip","nu1_scaled_dip","t1_scaled_gen","nuCP_scaled_dip","nuGCT_scaled_dip","m_fraction")
scaled_popt=(Nanc,nu1_scaled_dip,t1_scaled_gen,nuCP_scaled_dip,nuGCT_scaled_dip,m_fraction)
