...
modelName="2D.ConstantSize.asymMig.fixedT"
...
#CK: increasing pts_l since high migration can cause issues: https://groups.google.com/g/dadi-user/c/esRqfOQ7Amc?pli=1
pts_l = [ns[0]+20,ns[0]+40,ns[0]+60] # need 6 points because two populations 
...
def split_Wmig(params, ns, pts): 
    ...
    phi = Integration.two_pops(phi, xx, 4.4e-4, nu1, nu2, m12, m21)  # two pops at diff sizes with symmetric migration
    # allow for another size change; fix time at 35 gen
    fs = Spectrum.from_phi(phi, ns, (xx,xx)) 
    return fs
param_names=("nu1","nu2","m12","m21")
...
upper_bound = [1, 1, 20000, 20000] # 20 as upper bound on mig rec by dadi
p0 = [0.1, 0.1, 2000, 2000] # initial parameters; changing starting mu parameter because won't get permuted 
...
m21_fraction=popt[3]/(2*Nanc) # migration rates come out scaled by fraction of members of pop  scaled * 2Na according to dadi manual so here you actually DIVIDE by 2Na *not multiply* to get the migration fraction. Tricky tricky :)
# THIS IS TRICKY: confirmed here: https://groups.google.com/forum/#!topic/dadi-user/AY_4NoEjsx8
scaled_param_names=("Nanc_FromTheta_scaled_dip","nu1_scaled_dip","nu2_scaled_dip","m12Fraction","m21Fraction")
scaled_popt=(Nanc,nu1_scaled_dip,nu2_scaled_dip,m12_fraction,m21_fraction)
...
output=[popt_str,scaled_popt_str,theta,ll_model,func.__name__,mu,L,maxiter,runNum,todaysdate,p0,upper_bound,lower_bound] # put all the output terms together
